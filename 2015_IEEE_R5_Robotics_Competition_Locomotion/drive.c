...
void Robot_drive_task(void)
{
	...
	while(TRUE)
	{
		...
		test2++;
			...
			break;
		case DRIVESTATE_FWD:
			PIDError(prd, error, lightsnsr_val,0);					// Get error values
			...
			break;
		case DRIVESTATE_REV:
			PIDError(prd, error, lightsnsr_val,1);
 ...
 */
void PIDError(uint32_t periods[NUM_ARRAYS][NUM_SENSORS], int16_t error[NUM_ARRAYS][NUM_SENSORS], uint8_t * lightsense_vals, uint8_t indexsensor) // Consider tuning the weights (200,100,0) work well
{
	uint8_t n=0;
	for(n=0;n<NUM_ARRAYS;n++)
	{
		...
		if(lightsense_vals[indexsensor] ==0;)	// Check if see all white values (too far left/right)
		{
			if(error[n][1]<0) feedback = 0;						// if too far left, set feedback to 0
			if(error[n][1]>0) feedback = 2*WEIGHT;				// If too far right, set feedback to 200
		}
		error[n][0] = (int16_t)feedback - WEIGHT;				// Steady state is the center of the line
	}
}
...
{
	// We now use the center part of the PID sensors
	// If its white increment counter
	if(!(val[index_0] &= 0x2)) reads[0][NULLS]++;
	if(!(val[index_1] &= 0x2)) reads[1][NULLS]++;
	// Check if its black
	if(val[index_0] &= 0x2)
	{
		reads[0][BLACKS]++;
