...
void setEach(int32_t m1,int32_t m2,int32_t m3,int32_t m4)
{
	// Control Pins
	//M1:FWD, M2:FWD, M3:FWD, M4:FWD
	//M1:FWD, 1A hi, 1B lo
	...
	uint32_t period = PWMGenPeriodGet(PWM_MOTOR_BASE, PWM_GEN_TOPM);
	if (m1 != 0)	PWMPulseWidthSet(PWM_MOTOR_BASE, M1_OUT, (((m1 * period)/100) - 1));
	else			GPIOPinWrite(GPIO_PORTD_BASE, PD2_M1A | PD3_M1B, PD2_M1A | PD3_M1B);
	if (m2 != 0)	PWMPulseWidthSet(PWM_MOTOR_BASE, M2_OUT, (((m2 * period)/100) - 1));
	else			GPIOPinWrite(GPIO_PORTE_BASE, PE1_M2A | PE2_M2B | PE3_M3A, PE1_M2A | PE2_M2B | PE3_M3A);
	if (m3 != 0)	PWMPulseWidthSet(PWM_MOTOR_BASE, M2_OUT, (((m3 * period)/100) - 1));
	else 			GPIOPinWrite(GPIO_PORTA_BASE, PA5_M3B, PA5_M3B);
	if (m4 != 0)	PWMPulseWidthSet(PWM_MOTOR_BASE, M2_OUT, (((m4 * period)/100) - 1));
	else			GPIOPinWrite(GPIO_PORTB_BASE, PB4_M4A | PB5_M4B, PB4_M4A | PB5_M4B);
